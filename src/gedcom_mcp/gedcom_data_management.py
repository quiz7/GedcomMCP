...
def _add_person_internal(context: GedcomContext, name: str, gender: str) -> str:
...
def _create_marriage_internal(
    context: GedcomContext, husband_id: str, wife_id: str
) -> str:
...
def _add_child_to_family_internal(
    context: GedcomContext, child_id: str, family_id: str
) -> None:
...
def _remove_child_from_family_internal(
    context: GedcomContext, child_id: str, family_id: str
) -> str:
...
def _remove_event_internal(
    context, entity_id: str, event_type: str, date_to_match: str
) -> str:
    ...
    entity = (
        context.individual_lookup.get(entity_id)
        if is_person
        else context.family_lookup.get(entity_id)
    )
...
def _update_event_details_internal(
    context,
    entity_id: str,
    event_type: str,
    new_date: str = None,
    ...
    try:
        ...
        if not events:
            if (new_date is not None and new_date != "") or (
                new_place is not None and new_place != ""
            ):
                # Create new event
...
def batch_update_person_attributes(context: GedcomContext, updates: list) -> dict:
        ...
        try:
            ...
            if not isinstance(update, dict):
                ...
                results["errors"].append(
                    {"index": i, "error": "Update must be a dictionary"}
                )
                continue
            ...
            result = _update_person_attribute_internal(
                context, person_id, attribute_tag, new_value
            )
            if "Error" in result:
                results["failed"] += 1
                results["errors"].append(
                    {"index": i, "person_id": person_id, "error": result}
                )
            else:
...
def _add_note_to_entity_internal(
    context, entity_id: str, note_text: str = None, note_id: str = None
) -> str:
    ...
    entity = context.individual_lookup.get(entity_id) or context.family_lookup.get(
        entity_id
    )
    if not entity:
        return f"Error: Entity with ID {entity_id} not found."
...
def _update_person_attribute_internal(
    context, person_id: str, attribute_tag: str, new_value: str
) -> str:
...
def _remove_person_attribute_internal(
    context, person_id: str, attribute_tag: str
) -> str:
...
def _update_person_details_internal(
    context, person_id: str, name: Optional[str] = None, gender: Optional[str] = None
) -> str:
...
def _create_source_internal(
    context, title: str = "", author: str = "", publication: str = ""
):
...
def _delete_note_entity_internal(context, note_id: str) -> str:
    ...
    try:
        ...
        context.gedcom_parser.get_root_element().get_child_elements().remove(
            note_element
        )
