...
BIRTH_YEAR_PROXIMITY_THRESHOLD = (
    500  # How close a birth year must be to be considered "relevant" for the heuristic.
)
BIRTH_YEAR_HEURISTIC_FACTOR = 25.0  # The factor to scale the birth year distance by.
NO_BIRTH_YEAR_PENALTY = (
    9999  # A large value to deprioritize nodes without a birth year.
)
...
class NodePriority:
    ...
    def init_heuristics(self, gedcom_ctx):
        ...
        if self._birth_year_distance < BIRTH_YEAR_PROXIMITY_THRESHOLD:
            self._adjusted_distance += (
                self._birth_year_distance / BIRTH_YEAR_HEURISTIC_FACTOR
            )
    ...
    def __eq__(self, other):
        ...
        return (self._adjusted_distance, self._birth_year_distance, self.person_id) == (
            other._adjusted_distance,
            other._birth_year_distance,
            other.person_id,
        )
...
class GetCommonAncestorsParams(BaseModel):
    ...
    person_ids: List[str] = Field(
        description="List of person IDs to find common ancestors for"
    )
    max_level: int = Field(
        default=20, description="Maximum ancestor level to search (default: 20)"
    )
