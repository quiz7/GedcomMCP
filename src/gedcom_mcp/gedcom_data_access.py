            ...
            if tag == "NAME":
                ...
                for birt_child in child_elem.get_child_elements():
                    if birt_child.get_tag() == "DATE":
                        ...
                        birth_date = (
                            _normalize_genealogy_date(birth_date)
                            if birth_date
                            else birth_date
                        )
                        ...
                        birth_place = (
                            _normalize_genealogy_place(birth_place)
                            if birth_place
                            else birth_place
                        )
            elif tag == "DEAT":
                for deat_child in child_elem.get_child_elements():
                    if deat_child.get_tag() == "DATE":
                        ...
                        # Enhance with our date parsing
                        death_date = (
                            _normalize_genealogy_date(death_date)
                            if death_date
                            else death_date
                        )
                        ...
                        # Enhance with our place parsing
                        death_place = (
                            _normalize_genealogy_place(death_place)
                            if death_place
                            else death_place
                        )
            elif tag == "SEX":
...
def _get_person_relationships_internal(
    person_id: str, gedcom_ctx
) -> Optional[PersonRelationships]:
    """
...
def find_person_by_name(name: str, gedcom_ctx: GedcomContext) -> List[PersonDetails]:
    ...
    if hasattr(element, "get_child_elements"):
        ...
        for child in child_elements:
            ...
            if tag == "DATE":
                ...
                # Enhance with our date parsing
                date_value = (
                    _normalize_genealogy_date(date_value) if date_value else date_value
                )
                ...
                # Enhance with our place parsing
                place_value = (
                    _normalize_genealogy_place(place_value)
                    if place_value
                    else place_value
                )
...
def _get_events_internal(person_id: str, gedcom_ctx) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for element in root_child_elements:
            if element.get_pointer() == person_id and isinstance(
...
def _get_places_internal(
    query: Optional[str] = None, gedcom_ctx: Optional[GedcomContext] = None
) -> List[Dict[str, Any]]:
                ...
                if marriages:
                    for marriage in marriages:
                        ...
                        if marriage_place:
                            ...
                            places[normalized_name]["original_names"].add(
                                marriage_place
                            )
                            places[normalized_name]["occurrences"] += 1
...
def _get_person_attributes_internal(
    person_id: str, gedcom_ctx: GedcomContext
) -> Dict[str, str]:
...
def search_gedcom(
    ...
    search_type: str = "all",
    event_types: Dict[str, Any] = EVENT_TYPES,
) -> Dict[str, Any]:
                ...
                if hasattr(family_elem, "get_child_elements"):
                    ...
                    if query_lower in family_text.lower():
                        results["families"].append(
                            {"id": element_id, "type": "family", "match": "content"}
                        )
                ...
                if hasattr(element, "get_child_elements"):
                    for child_elem in element.get_child_elements():
                        ...
                        if search_type in ["all", "events"] and tag in event_types:
                            ...
                            if normalize_string(query) in normalize_string(
                                event_info["name"]
                            ) or normalize_string(query) in normalize_string(
                            ...
                            # Also search in event details (date, place, note)
                                ...
                                if hasattr(child_elem, "get_child_elements"):
                                    for event_detail in child_elem.get_child_elements():
                                        if event_detail.get_tag() in [
                                            "DATE",
                                            "PLAC",
                                            "NOTE",
                                        ]:
                                            event_text += " " + str(
                                                event_detail.get_value()
                                            )
...
def fuzzy_search_records(
    name: str, gedcom_ctx: GedcomContext, threshold: int = 80, max_results: int = 50
) -> list:
    ...
    for match_name, score in results:
        if score >= threshold:
            ...
            if person:
                matches.append(
                    {"person": person.model_dump(), "similarity_score": score}
                )
